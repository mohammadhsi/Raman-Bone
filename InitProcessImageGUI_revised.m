...
while(sum((npeaklambdaold==npeaklambda) + floor((isnan(npeaklambdaold)+isnan(npeaklambda))./2))<npeaknum)
    ...
    figure_handle = openfig('C:\Users\Mohammad\Box\GitHub\RamanProcessGUI\neon.fig'); % figure is opened
    %%
...
% Calculate correction factor using white lamp ----- SMH---- Feb. 12 2024
% WhiteLampCF contains the correction factor for each row

RowLengthWL = size(whitelamp, 1); % Number of rows in the matrix
ColLengthWL = size(whitelamp, 2); % Number of columns in the matrix

% Preallocate the matrix to store correction factors for each row
WhiteLampCF = zeros(RowLengthWL, ColLengthWL);

% Window size for smoothing
WindowSize = 200;

% Loop through each row
for Row = 1:RowLengthWL
    % Extract the current row
    WhiteSingleSpec = double(whitelamp(Row,:));
    
    % Create a smoothed version of the current row
    WhiteSmoothedSpec = smooth(WhiteSingleSpec, WindowSize)';
    
    % Calculate the correction factor for the current row
    CorrectionFactor = WhiteSingleSpec ./ WhiteSmoothedSpec;
    
    % Store the correction factor in the matrix
    WhiteLampCF(Row, :) = CorrectionFactor;
end
...
% Find the brightest row in green glass and then calcaulate coorection
% factor using this row data ------SMH ---- Feb. 12/2024

% Calculate the sum of each row
rowSums = sum(throughput, 2); % The '2' indicates that we sum along the 2nd dimension (rows)

% Find the row with the maximum sum
[maxValue, brightestRow] = max(rowSums);

% brightestRow now contains the index of the brightest row

ThroughputSingleSpec =  double(throughput(brightestRow,:));
% create a smoothed version that has no ripples
WindowSize = 200;
ThroughputSmoothedSpec = (smooth(double(ThroughputSingleSpec),WindowSize))';
...
% correction vector for green glass
CorrectionFactorGG = ISRM'./ThroughputSmoothedSpec;
...
while(sum((typeakwavenumold==typeakwavenum) + floor((isnan(typeakwavenumold)+isnan(typeakwavenum))./2))<typeaknum)
    ...
    figure_handle = openfig('C:\Users\Mohammad\Box\GitHub\RamanProcessGUI\tylenol.fig'); % figure is opened
    %%
    axes_handle = findobj(figure_handle, 'Type', 'Axes'); % find handle to axes in figure
    axes_children_handle = get(axes_handle, 'Children');
    copyobj(axes_children_handle, handles.previousstep); % children of original axes are copied to new axes
